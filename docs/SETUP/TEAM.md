...
(任意)コミット前検知(pre-commit)の設定を全リポジトリに強制する場合は、以下も追加する:

| Repository | Branch | Workflow |
|:-----------|:-------|:---------|
| `github-workflows` | `main` | `.github/workflows/pre-commit-check.yml` |

※ `pre-commit-check` は必須ワークフローのため全 PR で起動され、リポジトリ直下の `.pre-commit-config.yaml` に gitleaks hook が設定されているかを確認する。未設定のリポジトリでは PR コメントで導入手順(コピペ用スニペット込み)を案内し fail する。設定は各リポジトリの開発者が自分で追加する(中央からの配布は行わない)。登録しない場合、pre-commit はリポジトリごとの任意導入となる(本リポジトリ直下の `.pre-commit-config.yaml` が実例)。
...
ファイル内容に対するコミット前のローカル検知は pre-commit + gitleaks が担う。
