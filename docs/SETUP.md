...
[`workflow-templates/claude.yml`](../workflow-templates/claude.yml) を `.github/workflows/claude.yml` にコピーする(内容の変更は不要)。

- `uses:` が `@<sha>  # vX.Y.Z` の形になっている `main` の最新版からコピーする。タグ参照(`@vX.Y.Z`)のままだと zizmor / ghalint の必須ワークフローで落ちる
...
3. Renovate が `workflow-templates/claude.yml` と各リポジトリの `.github/workflows/claude.yml` の `uses:` を新しい SHA に更新する PR を作るので、承認してマージする
